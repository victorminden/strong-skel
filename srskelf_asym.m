% SRSKELF_ASYM   Unsymmetric strong recursive skeletonization factorization.
%
%    F = SRSKELF_ASYM(A,X,OCC,RANK_OR_TOL,PXYFUN) produces a factorization F of the
...
%
%    F = SRSKELF_ASYM(A,X,OCC,RANK_OR_TOL,PXYFUN,OPTS) also passes various options to
...
%
%      - SYMM: assume that the matrix is unsymmetric if SYMM = 'N' and Hermitian
...
function F = srskelf_asym(A,x,occ,rank_or_tol,pxyfun,opts)
  ...
  if opts.verb
      disp('This is standard unsymmetric srskelf (RS-S).');
  end
