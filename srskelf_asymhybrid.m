...
% SRSKELF_ASYMHYBRID   Asymmetric strong recursive skeletonization
% factorization with hybrid admissibility.
%
%    F = SRSKELF_ASYMHYBRID(A,X,OCC,RANK_OR_TOL,PXYFUN) produces a 
...
%
%    F = SRSKELF_ASYMHYBRID(A,X,OCC,RANK_OR_TOL,PXYFUN,OPTS) also passes 
...
%              decomposition; if SYMM = 'P', the Cholesky decomposition.
%
...
start = tic;
  ...
  if opts.verb
    disp('This is asymmetric srskelf with hybrid admissibility (RS-WS).');
    fprintf(['-'*ones(1,80) '\n'])
